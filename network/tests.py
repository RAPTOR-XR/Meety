...
# Local
from .models import Comment, React, Follow, UpPost, Profile, User
from .process import ProcProfile

class ModelsTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="test", password="test")
        self.post = UpPost.objects.create(user=self.user, content="test")
    ...
    def test_multiple_likes(self):
        ...
        React.objects.create(user=self.user, post=self.post, emoji_type=1)

        # Check if IntegrityError raised
        with self.assertRaises(IntegrityError):
            React.objects.create(user=self.user, post=self.post, emoji_type=2)
...
class FormsTestCase(TestCase):
    def test_user_profile_form_img_too_big(self):
        ...
        with open(test_img_path, "rb") as infile:
            ...
            form = ProcProfile(files={"image": img_file})
...
class ViewsTestCase(TestCase):
    ...
    def test_index_2_pages(self):
        ...
        for _ in range(11):
            UpPost.objects.create(user=self.user, content="test")
    ...
    def test_POST_post_comment_create_post(self):
        ...
        self.assertEqual(UpPost.objects.filter(content="post create test").count(), 1)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, '/')
    ...
    def test_POST_post_comment_create_comment(self):
        ...
        # Create a post
        post = UpPost.objects.create(user=self.user, content="test")
    ...
    def test_PUT_post_comment_edit_post(self):
        ...
        # Create a post
        old_post = UpPost.objects.create(user=self.user, content="old content")
        # Edit post's content
        ...
        new_post = UpPost.objects.get(pk=old_post.id)
    ...
    def test_DELETE_post_comment_delete_post(self):
        ...
        posts_count_before_delete = UpPost.objects.all().count()

        # Delete the post
        response = self.c.delete('/post-comment/post', json.dumps({"id": post.id}))

        # Get posts count after delete request
        posts_count_after_delete = UpPost.objects.all().count()
    ...
    def test_user_profile_show_only_users_posts(self):
        ...
        # Create user's post
        post_user = UpPost.objects.create(user=self.user, content="user")
        # Create user_2's post
        post_user_2 = UpPost.objects.create(user=user_2, content="user_2")

        # Get all posts count
        all_posts = UpPost.objects.all()
    ...
    def test_GET_like_post_correct(self):
        ...
        post = UpPost.objects.create(user=self.user, content="test")
        # Crete a like
        React.objects.create(user=self.user, post=post, emoji_type=2)

        response = self.c.get(f'/like/post/{post.id}')

        # Get JSON reponse
        json_response = json.loads(response.content)
        response_emoji_type = [emoji_num for emoji_num, emoji_str in React.choices if emoji_str == json.loads(response.content)["emojiType"]][0]
    ...
    def test_GET_like_comment_correct(self):
        ...
        # Crete a like
        React.objects.create(user=self.user, comment=comment, emoji_type=2)
    ...
    def test_following_1_page(self):
        ...
        for _ in range(11):
            UpPost.objects.create(user=new_user, content="test")
    ...
    def test_following_show_only_users_posts(self):
        ...
        user_2 = User.objects.create_user(username="test_2", password="test_2")
        # Follow the new user
        Follow.objects.create(user=self.user, user_followed=user_2)

        # Create user's post
        UpPost.objects.create(user=self.user, content="user")
        # Create user_2's post
        post_user_2 = UpPost.objects.create(user=user_2, content="user_2")
...
class FrontEndTestCase(StaticLiveServerTestCase):
    def setUp(self):
        ...
        # Create a post
        post = UpPost.objects.create(user=self.user, content="post 1")
        # Create comments
        for i in range(3):
    ...
    def like_panel_test(self, user, post_comment):
        ...
        if post_comment.get_attribute("class").find("post") != -1:
            like = React.objects.filter(post=UpPost.objects.get(user=user)).all()
        else:
            comment_id = int(post_comment.get_attribute("id").split("_")[-1])
            like = React.objects.filter(comment=comment_id).all()
    ...
    def test_frontend_create_post_from_form(self):
        ...
        self.assertEqual(UpPost.objects.filter(content="Sellenium test").count(), 1)

    def test_frontend_post_order(self):
        """ Create 2 posts -> check if they are in correct order (from newest to oldest) """
        # Create 2 posts
        post_1 = UpPost.objects.create(user=self.user, content="post 2")
        time.sleep(0.1)
        post_2 = UpPost.objects.create(user=self.user, content="post 3")
    ...
    def test_frontend_following_data(self):
        ...
        for i in range(5):
            ...
            Follow.objects.create(user=users[i], user_followed=self.user)

        # Make self.user be followed by 2 users
        Follow.objects.create(user=self.user, user_followed=users[0])
        Follow.objects.create(user=self.user, user_followed=users[1])
    ...
    def test_frontend_follow_unfollow_button(self):
        ...
        follow_button.click()
        time.sleep(0.1)
        self.assertEqual(Follow.objects.filter(user=self.user, user_followed=new_user).count(), 1)
        ...
        time.sleep(0.1)
        self.assertEqual(Follow.objects.filter(user=self.user, user_followed=new_user).count(), 0)
    ...
    def test_frontend_post_content(self):
        ...
        new_user = User.objects.create_user(username="1", password="1")
        # Follow the user
        Follow.objects.create(user=self.user, user_followed=new_user)
        # Creat a post by the second user
        UpPost.objects.create(user=new_user, content="new user post")
    ...
    def test_frontend_edit_post(self):
        ...
        for url in ["", f"/user-profile/{self.user.id}"]:
            ...
            # Get the post's data
            edited_post = UpPost.objects.get(user=self.user)
    ...
    def test_frontend_index_delete_post(self):
        ...
        # Get the post
        deleted_post = UpPost.objects.filter(user=self.user).all()
    ...
    def test_frontend_showmore_button_post(self):
        ...
        UpPost.objects.create(user=self.user, content=("Lorem ipsum" * 200))
        ...
        UpPost.objects.create(user=new_user, content=("Lorem ipsum"))
        time.sleep(0.5)
        # Creat a long text post (new_user)
        UpPost.objects.create(user=new_user, content=("Lorem ipsum" * 200))
    ...
    def test_frontend_index_change_like_post(self):
        ...
        # Create a like
        React.objects.create(user=self.user, post=self.user.posts.all()[0], emoji_type=1)
    ...
    def test_frontend_following_change_like_post(self):
        ...
        Follow.objects.create(user=self.user, user_followed=new_user)
        # Creat a post
        new_post = UpPost.objects.create(user=new_user, content="Lorem ipsum")
        # Create a like
        React.objects.create(user=self.user, post=new_post, emoji_type=1)
    ...
    def test_frontend_index_like_emoji_twice_post(self):
        ...
        new_user = User.objects.create_user(username="1", password="1")
        # Create likes
        React.objects.create(user=self.user, post=self.user.posts.all()[0], emoji_type=2)
        React.objects.create(user=new_user, post=self.user.posts.all()[0], emoji_type=2)
    ...
    def test_frontend_user_profile_like_emoji_twice_post(self):
        ...
        new_user = User.objects.create_user(username="1", password="1")
        # Create likes
        React.objects.create(user=self.user, post=self.user.posts.all()[0], emoji_type=3)
        React.objects.create(user=new_user, post=self.user.posts.all()[0], emoji_type=3)
    ...
    def test_frontend_following_like_emoji_twice_post(self):
        ...
        # Create likes
        React.objects.create(user=self.user, post=new_post, emoji_type=1)
        React.objects.create(user=new_user, post=new_post, emoji_type=1)
    ...
    def test_frontend_index_emoji_correct_order_post(self):
        ...
        # Create likes
        React.objects.create(user=self.user, post=self.user.posts.all()[0], emoji_type=1)
        React.objects.create(user=second_user, post=self.user.posts.all()[0], emoji_type=3)
        React.objects.create(user=third_user, post=self.user.posts.all()[0], emoji_type=3)
    ...
    def test_frontend_following_emoji_correct_order_post(self):
        ...
        Follow.objects.create(user=self.user, user_followed=second_user)
        # Creat a post by the second user
        new_post = UpPost.objects.create(user=second_user, content="Lorem ipsum")
        # Create likes
        React.objects.create(user=self.user, post=new_post, emoji_type=3)
        React.objects.create(user=second_user, post=new_post, emoji_type=1)
        React.objects.create(user=third_user, post=new_post, emoji_type=3)
    ...
    def test_frontend_comment_creator_button_text(self):
        ...
        Follow.objects.create(user=self.user, user_followed=new_user)
        # Creat a post by the second user
        post = UpPost.objects.create(user=new_user, content="new user post")
    ...
    def test_frontend_following_like_comment(self):
        ...
        Follow.objects.create(user=self.user, user_followed=new_user)
        # Creat a post
        post = UpPost.objects.create(user=new_user, content="Lorem ipsum")
        # Creat a comment
    ...
    def test_frontend_index_change_like_comment(self):
        ...
        # Create a like
        React.objects.create(user=self.user, comment=comment, emoji_type=1)
    ...
    def test_frontend_index_like_emoji_twice_comment(self):
        ...
        # Create likes
        React.objects.create(user=self.user, comment=comment, emoji_type=2)
        React.objects.create(user=new_user, comment=comment, emoji_type=2)
    ...
    def test_frontend_index_emoji_correct_order_comment(self):
        ...
        # Create likes
        React.objects.create(user=self.user, comment=comment, emoji_type=1)
        React.objects.create(user=second_user, comment=comment, emoji_type=3)
        React.objects.create(user=third_user, comment=comment, emoji_type=3)
    ...
    def test_frontend_following_emoji_correct_order_comment(self):
        ...
        Follow.objects.create(user=self.user, user_followed=second_user)
        # Creat a post
        post = UpPost.objects.create(user=second_user, content="Lorem ipsum")
        # Creat a comment
